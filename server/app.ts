...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url?.startsWith('/pokemons2/')) {
    const name = req.url?.replace('/pokemons2/', '');
  ...
  if (req.url?.startsWith('/pokemons2')) {
    ...
    try {
      ...
      responseData.next = pokemonData.next?.replace('https://pokeapi.co/api/v2/pokemon', 'localhost:3000/pokemons2');
      responseData.previous = pokemonData.previous?.replace(
        'https://pokeapi.co/api/v2/pokemon',
        'localhost:3000/pokemons2',
      );
